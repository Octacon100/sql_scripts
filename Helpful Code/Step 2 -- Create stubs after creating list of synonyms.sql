...
SET @databasename = N'CFN_SECURITY';
    ...
    BEGIN
        ...
        ELSE IF @object_type = 'P'
                 BEGIN
                     ...
                     SELECT 'CREATE OR ALTER ' + ROUTINE_TYPE + ' stub.' + ROUTINE_NAME
        ...
        ELSE IF @object_type IN ( 'FN', 'IF' )
                 BEGIN
                     ...
                     SELECT 'CREATE OR ALTER ' + ROUTINE_TYPE + ' stub.' + ROUTINE_NAME + ' '
                            + SUBSTRING(
                                           ROUTINE_DEFINITION,
                                           PATINDEX('%(%', ROUTINE_DEFINITION),
                                           PATINDEX('%AS%', ROUTINE_DEFINITION) - PATINDEX('%(%', ROUTINE_DEFINITION)
                                       ) + ' AS '
                            + RIGHT(ROUTINE_DEFINITION, PATINDEX('%NRUTER%', REVERSE(ROUTINE_DEFINITION))
                                                        + LEN('RETURN')) AS sql,
                            ROUTINE_NAME,
                            @object_type
                     FROM INFORMATION_SCHEMA.ROUTINES
